...
<p><br>
		...
		<form name="frm" role="form" method="get" action="01_workflow_step0.php">
						<label>작업 유형 선택</label>
							<select class="form-control" name = 'indi_tem_idx'>
							<?php
								$admin_company_info_sql	 = "select * from individual_template_main;";
								...
								while($admin_company_info_info	 = mysqli_fetch_array($admin_company_info_res)){
										echo "<option value = '".$admin_company_info_info['indi_tem_idx']."'>".$admin_company_info_info['indi_tem_title']."</option>";
								};
							?>
							</select>

							<p><br><p><br>
							<input type = 'hidden' name='mails' value='1'>		
					<input  type='submit' class="btn btn-success login-btn" type="submit" value="지라 생성하러 가기">
				</form>

				<p><br><p><br>		<p><br><p><br>		<p><br><p><br>		<p><br><p><br>
<?php
$url = 'https://gmail.googleapis.com/gmail/v1/users/yoonhd@mz.co.kr/messages/17fac1d7c5a6ba5e?access_token=ya29.A0ARrdaM8jUXJeTC6U6neCQdxsFRI5dqu2cRtftvQB8_KP1wMg8nWIQVNiymMJu7lhp6gBu7CiWOZFKXAVD5bgY8yI2ICu_QIBc_5ZqbXUoDejwbQWFFHP2KlAl4xvACV7skJsxIDV0SKQP4t-OpiSuHtcUlXH';
$response = file_get_contents($url);
$object = json_decode($response,true);


echo "메일 제목: ".$object['payload']['headers'][20]['value']."<p><p>";
echo "메일 내용 : ".$object['snippet']."<p>";
echo "보낸이 : ".$object['payload']['headers'][17]['value']."<p>";
echo "받은시간: ".$object['payload']['headers'][1]['value']."<p>";
?>
<p><br>
		...
		<form name="frm" role="form" method="get" action="01_workflow_step0.php">
						<label>작업 유형 선택</label>
							...
							<p><br><p><br>
							<input type = 'hidden' name='mails' value='2'>		
					<input  type='submit' class="btn btn-success login-btn" type="submit" value="지라 생성하러 가기">
				</form>
